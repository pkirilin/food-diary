...
- Done in 04: the `get_food_logs` half of case 2 — `The_client_can_get_food_logs` seeds a note through `Given_notes`, lists tools with a real `McpClient` and calls the listed tool. `The_client_is_told_to_split_food_logs_range_wider_than_31_days` asserts the cap arrives as `isError`. `Then_food_logs_contain` asserts only that `tools/list` contains `get_food_logs`; widen it to both tools once 05 lands.
