...
- `src/frontend/` — React 19 + TypeScript SPA (Vite, MUI v6, Redux Toolkit + RTK Query, react-router v7, react-hook-form + Zod). Uses **Feature-Sliced Design**: `app/`, `pages/`, `widgets/`, `features/`, `entities/`, `shared/`. Respect FSD import direction (upper layers may import from lower; never the reverse).
